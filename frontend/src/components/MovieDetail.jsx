...
const MovieDetail = () => {
  ...
  // 👈 2. Extract Context functions & watchlist array
  ...
  // Scroll to top instantly whenever movie ID or route changes
  useEffect(() => {
    window.scrollTo(0, 0);
    document.documentElement.scrollTop = 0;
    document.body.scrollTop = 0;
  ...
  // 👈 3. Sync Favorite state dynamically with AppContext & LocalStorage
  useEffect(() => {
    if (!movie) return;
    
    // Check in AppContext first, then check LocalStorage fallback
    const inContext = watchlist.some((item) => String(item.id || item.movieId) === String(movie.id));
    
    const currentFavs = JSON.parse(localStorage.getItem('favMovies')) || JSON.parse(localStorage.getItem('watchlist')) || [];
    const inLocalStorage = currentFavs.some(item => String(item.id || item.movieId) === String(movie.id));

    setIsFavorite(inContext || inLocalStorage);
  }, [movie, watchlist]);
  ...
  // 👈 4. Updated toggle handler (Database + State Sync)
  const toggleFavorite = async () => {
    ...
    if (isFavorite) {
      // Remove via Context (Backend API call)
      if (removeFromWatchlist) {
        await removeFromWatchlist(movie.id);
      }
      currentFavs = currentFavs.filter(item => String(item.id || item.movieId) !== String(movie.id));
      setIsFavorite(false);
    } else {
      // Add via Context (Backend API call + DB insert)
      if (addToWatchlist) {
        await addToWatchlist(formattedMovie);
      }
      currentFavs.push(formattedMovie);
      setIsFavorite(true);
    }

    // Sync to both localStorage keys as fallback
    localStorage.setItem('favMovies', JSON.stringify(currentFavs));
    localStorage.setItem('watchlist', JSON.stringify(currentFavs));
